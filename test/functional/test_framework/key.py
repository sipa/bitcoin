...
class ECKey():
    ...
    def sign_schnorr(self, msg):
        """Construct a Schnorr signature (see BIP 340) with this key."""
        assert(self.valid)
...
def verify_schnorr(key, sig, msg):
    ...
    This is a separate function rather than an ECPubKey method, because
    Schnorr signature verification is defined to accept a byte array as
    public key rather than an EC point.
    """
    assert(len(key) == 32)
    assert(len(msg) == 32)
    assert(len(sig) == 64)
    ...
    R = SECP256K1.mul([(SECP256K1_G, s), (point.p, SECP256K1_ORDER - e)])
    if jacobi_symbol(R[1] * R[2], SECP256K1_FIELD_SIZE) != 1:
        return False
