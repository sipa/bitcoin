...
# the optimal (period, bufsize) changes. These are filtered to keep only the ones with a strictly
# unique optimum (no two (period, bufsize) configurations tie on memory usage), and the ones where
# the iterative simulation here and the closed-form approximation it is replaced with later agree.
# Among those, the transition whose max_headers and minchainwork_headers are closest to integers is
# picked. The result is a set of test vectors that are close to values where the results change.
