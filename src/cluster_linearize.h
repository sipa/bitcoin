...
class SFL2
{
    ...
    SetIdx Activate(TxIdx parent, TxIdx child) noexcept
    {
        ...
        UpdateDeps<false>(top_set_data.Set(), parent, bottom_set_data.setinfo);
    ...
    std::pair<SetIdx, SetIdx> Deactivate(TxIdx parent, TxIdx child) noexcept
    {
        ...
        for (auto tx : top_set_data.Set()) m_tx_data[tx].chunk_set = top_set;
    ...
    SetIdx MergeStep(SetIdx chunk) noexcept
    {
        ...
        if (best_other_chunk == INVALID_SET_IDX) return INVALID_SET_IDX;
        if constexpr (DownWard) {
            chunk = MergeChunks(chunk, best_other_chunk);
        } else {
            chunk = MergeChunks(best_other_chunk, chunk);
        }
        Assume(chunk != INVALID_SET_IDX);
        return chunk;
    }

    /** Perform an upward or downward merge sequence on the specified chunk. Returns the
