...
private:
    ...
    /** Structure with information about a single transaction. The most frequently accessed
     *  fields come first, the large dep_top_idx array last. */
    struct TxData {
    ...
    SetIdx MergeChunks(SetIdx top_idx, SetIdx bottom_idx) noexcept
    {
        ...
        // intersections.
        unsigned num_deps{0};
        std::array<uint8_t, SetType::Size()> counts;
        unsigned pos{0};
