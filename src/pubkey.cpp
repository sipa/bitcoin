...
bool XOnlyPubKey::CheckTapTweak(const XOnlyPubKey& base, const uint256& merkle_root, bool parity) const
{
    secp256k1_xonly_pubkey base_point;
    if (!secp256k1_xonly_pubkey_parse(secp256k1_context_verify, &base_point, base.data())) return false;
    uint256 tweak = base.ComputeTapTweakHash(&merkle_root);
    return secp256k1_xonly_pubkey_tweak_add_check(secp256k1_context_verify, m_keydata.begin(), parity, &base_point, tweak.begin());
}
