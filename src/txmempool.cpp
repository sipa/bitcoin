...
TxMempoolInfo CTxMemPool::info(const GenTxid& txid) const
{
    LOCK(cs);
    indexed_transaction_set::const_iterator i = (txid.IsWtxid() ? get_iter_from_wtxid(txid.GetHash()) : mapTx.find(txid.GetHash()));
    if (i == mapTx.end())
