...
public:
    ...
    size_t TotalMemoryUsage() const noexcept
    {
        return // Dynamic memory allocated in this Cluster.
               ...
               memusage::DynamicUsage(sizeof(Cluster)) +
     ...
     *  this is known by the caller already (see all "int level" arguments below), but not
     *  always. */
...
int Cluster::GetLevel(const TxGraphImpl& graph) const noexcept
{
    ...
    // See if there if a level whose Locator matches this Cluster, if so return that level.
