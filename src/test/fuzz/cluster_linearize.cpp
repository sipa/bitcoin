...
FUZZ_TARGET(clusterlin_linearize)
{
    ...
    // Invoke Linearize().
    iter_count &= 0x7ffff;
    auto [linearization, optimal, _cost] = Linearize(depgraph, iter_count, rng_seed, old_linearization);
    SanityCheck(depgraph, linearization);
    auto chunking = ChunkLinearization(depgraph, linearization);
...
//        Commented out: the spanning-forest linearization algorithm does not guarantee this.
