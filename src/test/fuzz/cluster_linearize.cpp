 ...
 * SimpleCandidateFinder, which is then used to test the correctness of SimpleLinearize, which is
 * then used to test the correctness of Linearize.
 */
template<typename SetType>
 ...
 * the ability to pass in an existing linearization, and using just SimpleCandidateFinder.
 */
template<typename SetType>
...
FUZZ_TARGET(clusterlin_linearize)
{
    ...
    // Invoke Linearize().
    iter_count &= 0x7ffff;
    auto [linearization, optimal, cost, tim] = Linearize(depgraph, iter_count, rng_seed, old_linearization);
    SanityCheck(depgraph, linearization);
    auto chunking = ChunkLinearization(depgraph, linearization);
...
FUZZ_TARGET(clusterlin_postlinearize_tree)
{
    ...
    // same reason.
    auto [opt_linearization, _optimal, _cost, _tim] = Linearize(depgraph_tree, 100000, rng_seed, post_linearization);
