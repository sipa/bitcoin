...
CAddrInfo* CAddrMan::Find(const CNetAddr& addr, int64_t* pnId)
{
    AssertLockHeld(cs);
...
CAddrInfo* CAddrMan::Create(const CAddress& addr, const CNetAddr& addrSource, int64_t* pnId)
{
    AssertLockHeld(cs);

    int64_t nId = nIdCount++;
...
void CAddrMan::SwapRandom(unsigned int nRndPos1, unsigned int nRndPos2)
{
    ...
    int64_t nId1 = vRandom[nRndPos1];
    int64_t nId2 = vRandom[nRndPos2];
...
void CAddrMan::Delete(int64_t nId)
{
    AssertLockHeld(cs);
...
void CAddrMan::ClearNew(int nUBucket, int nUBucketPos)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != -1) {
        int64_t nIdDelete = vvNew[nUBucket][nUBucketPos];
...
void CAddrMan::MakeTried(CAddrInfo& info, int64_t nId)
{
    AssertLockHeld(cs);
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        int64_t nIdEvict = vvTried[nKBucket][nKBucketPos];
    ...
    if (!newOnly &&
       (nTried > 0 && (nNew == 0 || insecure_rand.randbool() == 0))) {
        ...
        while (1) {
            ...
            int64_t nId = vvTried[nKBucket][nKBucketPos];
        ...
        while (1) {
            ...
            int64_t nId = vvNew[nUBucket][nUBucketPos];
            assert(mapInfo.count(nId) == 1);
            CAddrInfo& info = mapInfo[nId];
            if (insecure_rand.randbits(30) < fChanceFactor * info.GetChance() * (1 << 30))
                return info;
...
int CAddrMan::Check_()
{
    ...
    std::unordered_set<int64_t> setTried;
    std::unordered_map<int64_t, int> mapNew;

    if (vRandom.size() != (size_t)(nTried + nNew))
        return -7;

    for (const auto& entry : mapInfo) {
        int64_t n = entry.first;
...
void CAddrMan::ResolveCollisions_()
{
    ...
    for (std::set<int64_t>::iterator it = m_tried_collisions.begin(); it != m_tried_collisions.end();) {
        int64_t id_new = *it;
        ...
        if (mapInfo.count(id_new) != 1) {
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                int64_t id_old = vvTried[tried_bucket][tried_bucket_pos];
...
CAddrInfo CAddrMan::SelectTriedCollision_()
{
    ...
    std::set<int64_t>::iterator it = m_tried_collisions.begin();
