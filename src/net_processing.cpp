...
static TxRequestTracker g_txrequest GUARDED_BY(cs_main) {INBOUND_PEER_TX_DELAY, GETDATA_TX_INTERVAL, MAX_PEER_TX_IN_FLIGHT};
...
void RequestTx(CNode* node, const uint256& txid, std::chrono::microseconds current_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    g_txrequest.ReceivedInv(node->GetId(), !node->fInbound, txid, current_time);
}
...
void PeerLogicValidation::FinalizeNode(NodeId nodeid, bool& fUpdateConnectionTime) {
    ...
    g_txrequest.DeletedNode(nodeid);
    nPreferredDownload -= state->fPreferredDownload;
